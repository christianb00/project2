...
int main(){
        ...
        cout<<"\nMenu"<<
                ...
                "\nChoice 4: Determinant(up to 3x3 dimension matrix)"<<
                "\nChoice 5: Transpose"<<
                "\nChoice 6: Inverse"<<
                "\nChoice 7: Resize"
                "\nChoice 8: Reshape"
...
void getCofactor(int* ptr, int* tPtr, int p, int q, int n){
...
int matDet(matrix *arr){
        ...
        if(n == 1) {
                ...
                det = 0;
                for(int j = 0; j<n; j++) {
                        det += pow(-1,j) * *(A+j)*matDet(minorMat(arr,j));
                }
                return det;
        }
}

matrix* minorMat(matrix* arr, int cur){
        int n = arr->rows;
        int *aMin = new int[(n-1) * (n-1)];
        int *ptr = arr->ptr;
...
void matTran(matrix *arr){
        matrix *transp = new matrix;
        int *tPtr = new int[arr->rows*arr->cols];
